...
class DeliveryMan extends Authenticatable
{
    ...
    protected $appends = ['image_full_url','identity_image_full_url','additional_documents_full_url'];
    ...
    {
        return $this->belongsToMany(Shift::class, 'delivery_man_shift');
    }

    public function checkShiftValidity()
    {
        $shifts = $this->shifts;
        $current_time = now()->format('H:i:s');

        // 1. Check Full Day
        if ($shifts->contains('is_full_day', 1)) {
            return $this->shifts->where('is_full_day', 1)->first();
        }

        // 2. Check Specific Shifts
        foreach ($shifts as $shift) {
            if ($shift->start_time <= $current_time && $shift->end_time >= $current_time) {
                return $shift;
            }
        }
            ...
            foreach ($value as $group => $items) {
                if (is_array($items)) {
                    foreach ($items as $item) {
                        ...
                        if ($file) {
                            $images[] = Helpers::get_full_url('additional_documents/dm', $file, $storage);
                        }
                    }
                }
            }
