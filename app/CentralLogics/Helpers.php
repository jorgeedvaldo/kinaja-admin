...
class Helpers
{
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                foreach (json_decode($item?->category_ids) as $value) {
                    $categories[] = ['id' => (string) $value->id, 'position' => $value->position, 'category_name' => Category::find($value->id)?->name];
                }
    ...
    {
        if (! $dmwallet = DeliveryManWallet::firstOrNew(['delivery_man_id' => $delivery_man_id])) {
            return false;
        }
    ...
    {
        return [
            'vendor_list', 'category_list', 'campaign', 'cuisine_list', 'home_page', 'contact_us_page', 'about_us_page', 'restaurant_join_page', 'deliveryman_join_page', 'terms_and_conditions_page', 'privacy_policy_page', 'refund_policy_page', 'cancellation_policy_page', 'shipping_policy_page',
        ];
    }
