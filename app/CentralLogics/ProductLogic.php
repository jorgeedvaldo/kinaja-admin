...
class ProductLogic
{
    ...
    {
        if($campaign == true){
            return ItemCampaign::with('foodSeoData')->find($id);
        }
        return Food::active()
                    ->with([
                        'foodSeoData',
                        'reviews' => function ($query) {
                            $query->select('id', 'food_id', 'rating', 'comment', 'user_id', 'created_at')
                                ->where('status', 1);
                        }
                    ])
    ...
    public static function most_reviewed_products($zone_id, $limit = null, $offset = null, $type='all',$longitude=0,$latitude=0)
    {
        ...
        if($limit != null && $offset != null)
        {
            $query = Food::Active()->with('restaurant')
                ...
                ->withCount('reviews')->type($type)->having('reviews_count', '>', 0);
